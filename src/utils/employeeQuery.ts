...
export interface EmployeeQueryFilters {
  ...
  sortBy?: "MonthlySalary" | "PerformanceRating" | "AttritionRiskScore" | "OvertimeHours";
...
export const queryEmployeesToolDefinition = {
  ...
  description:
    "Search the full HR dataset (25,000 monthly employee records) with filters and return matching rows. Use this for anything the aggregated digest doesn't cover: cross-tabulating multiple fields (e.g. high performance rating AND low salary), listing or counting specific employees, looking up a person by name or ID, or any precise row-level question. Combine multiple filters in one call to answer compound questions. Returns up to `limit` matching rows plus the total match count.",
  input_schema: {
    ...
    properties: {
      ...
      sortBy: {
        type: "string",
        description: "Field to sort by: MonthlySalary, PerformanceRating, AttritionRiskScore, or OvertimeHours.",
      },
